#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 12.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 12 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-12.conf" ] || die 'manifesto da Etapa 12 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-xdg-application-index/Cargo.toml" ] || die 'crate nexxus-xdg-application-index ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando XDG Application Index em release'
    run_logged cargo build --workspace --release || die 'cargo build falhou'
    log_msg '[test] executando fmt, clippy, testes e rustdoc'
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação do XDG Application Index falhou'
}
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: biblioteca/serviço integrável sem payload executável independente nesta etapa'
