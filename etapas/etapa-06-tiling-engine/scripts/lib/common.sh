#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 06.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 06 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-06.conf" ] || die 'manifesto da Etapa 06 não encontrado'
    [ -f "$ROOT_DIR/../etapa-02-window-manager-core/crates/nexxus-wm/Cargo.toml" ] || die 'dependência nexxus-wm ausente'
    [ -f "$ROOT_DIR/../etapa-05-workspace-manager/crates/nexxus-workspaces/Cargo.toml" ] || die 'dependência nexxus-workspaces ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Tiling Engine em release'
