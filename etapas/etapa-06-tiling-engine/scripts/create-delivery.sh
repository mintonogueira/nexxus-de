#!/bin/sh
# Gera snapshot versionável da Etapa 06 após as validações técnicas.
set -eu

SCRIPT_DIR=$(dirname "$0"); case "$SCRIPT_DIR" in -*) SCRIPT_DIR="./$SCRIPT_DIR" ;; esac
ROOT_DIR=$(CDPATH= cd "$SCRIPT_DIR/.." && pwd)
. "$ROOT_DIR/manifests/etapa-06.conf"
DELIVERY_DIR="$ROOT_DIR/entrega"
ARCHIVE="$DELIVERY_DIR/Nexxus_Etapa06_Tiling_Engine_${NEXXUS_VERSION}.tar.gz"
