#!/bin/sh
# Pipeline Debian da Etapa 07: autoprovisiona, compila, testa, mede e prepara
# staging sem instalar conteúdo diretamente no host.
set -eu
...
install_missing_debian_build_deps() {
    _missing=''
    for _pkg in $DEBIAN_BUILD_PACKAGES; do dpkg-query -W -f='${Status}' "$_pkg" 2>/dev/null | grep -q 'ok installed' || _missing="$_missing $_pkg"; done
...
die 'payload instalável não está definido para a Etapa 07'
