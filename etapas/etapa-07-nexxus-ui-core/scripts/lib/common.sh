#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 07.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 07 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-07.conf" ] || die 'manifesto da Etapa 07 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-ui/Cargo.toml" ] || die 'crate nexxus-ui ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Nexxus UI Core em release'
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: biblioteca interna + harness de desenvolvimento sem payload runtime independente'
    log_msg '[status] build/test/demo/footprint/staging concluídos; pacote nativo e instalação permanecem N/A'
}
