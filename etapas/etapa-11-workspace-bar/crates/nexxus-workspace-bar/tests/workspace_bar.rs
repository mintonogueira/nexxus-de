...
fn incremental_events_update_visible_state_only() {
    ...
    assert!(model.apply_event(&WorkspaceEvent::Activated {
        previous,
        current: mail
    }));
    assert_eq!(model.active_id(), Some(mail));
...
fn pointer_release_emits_workspace_or_settings_action() {
    ...
    let id = nexxus_workspaces::WorkspaceId::new(1).unwrap();
    let layout =
        WorkspaceBarLayout::build(&[monitor], &[(id, "WEB")], WorkspaceBarMetrics::default())
            .unwrap();
    ...
    assert_eq!(
        state.pointer_release(&layout, point),
        Some(WorkspaceBarAction::Activate(id))
    );
    ...
    assert_eq!(
        state.pointer_release(&layout, settings),
        Some(WorkspaceBarAction::OpenWorkspaceSettings)
    );
    assert_eq!(
        InteractionState::hit_test(&layout, settings),
        Some(WorkspaceBarTarget::Settings)
    );
}
...
fn painter_renders_hidpi_frame_with_workspace_asset() {
    ...
    let mut painter =
        WorkspaceBarPainter::new(Theme::default(), metrics, AssetSource::new(asset_root));
    let frame = painter
    ...
    assert_eq!(
        frame.pixels.len(),
        frame.size.width as usize * frame.size.height as usize * 4
    );
}
