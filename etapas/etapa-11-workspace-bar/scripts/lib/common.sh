#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 11.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 11 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-11.conf" ] || die 'manifesto da Etapa 11 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-workspace-bar/Cargo.toml" ] || die 'crate nexxus-workspace-bar ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Workspace Bar em release'
    ...
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação da Workspace Bar falhou'
}
