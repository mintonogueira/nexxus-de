#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 02.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 02 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-02.conf" ] || die 'manifesto da Etapa 02 não encontrado'
...
build_and_test_workspace() {
    log_msg '[build] compilando Window Manager Core em release'
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: não existe payload runtime instalável'
