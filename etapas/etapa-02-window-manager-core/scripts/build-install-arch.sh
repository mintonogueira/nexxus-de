#!/bin/sh
# Pipeline Arch da Etapa 02: autoprovisiona dependências, compila, testa e
# prepara staging sem inventar pacote para uma biblioteca interna.
set -eu
...
. "$ROOT_DIR/manifests/etapa-02.conf"
...
install_missing_arch_build_deps() {
    _missing=''
    for _pkg in $ARCH_BUILD_PACKAGES; do pacman -Q "$_pkg" >/dev/null 2>&1 || _missing="$_missing $_pkg"; done
...
install_missing_arch_build_deps; validate_rust_toolchain; build_and_test_workspace; prepare_staging
[ "$NEXXUS_INSTALLABLE" = '0' ] && { finish_non_installable_stage; exit 0; }
die 'payload instalável não está definido para a Etapa 02'
