#!/bin/sh
# Pipeline Debian da Etapa 02: autoprovisiona dependências, compila, testa e
...
install_missing_debian_build_deps() {
    ...
    for _pkg in $DEBIAN_BUILD_PACKAGES; do
        dpkg-query -W -f='${Status}\n' "$_pkg" 2>/dev/null | grep -q '^install ok installed$' || _missing="$_missing $_pkg"
    done
...
install_missing_debian_build_deps; validate_rust_toolchain; build_and_test_workspace; prepare_staging
