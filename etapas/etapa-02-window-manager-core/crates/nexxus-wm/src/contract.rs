...
pub enum WmEvent {
    WindowCreated { id: WindowId, geometry: Geometry, constraints: SizeConstraints, metadata: WindowMetadata },
    WindowDestroyed { id: WindowId },
    WindowMapped { id: WindowId },
    WindowUnmapped { id: WindowId },
    WindowGeometryChanged { id: WindowId, geometry: Geometry },
    FocusChanged { id: Option<WindowId> },
    WindowMetadataChanged { id: WindowId, metadata: WindowMetadata },
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum WmCommand {
    RequestFocus { window: WindowId },
    RequestMove { window: WindowId, x: i32, y: i32 },
    RequestResize { window: WindowId, width: u32, height: u32 },
    RequestMaximize { window: WindowId },
    RequestRestore { window: WindowId },
    RequestFullscreen { window: WindowId, enabled: bool },
    RequestClose { window: WindowId },
}
