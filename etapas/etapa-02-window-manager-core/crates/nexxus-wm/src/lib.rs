...
pub use types::{
    Geometry, GeometryError, PresentationState, RestoreSnapshot, SizeConstraints, Window, WindowId,
    WindowIdError, WindowMetadata, WindowPlacement,
};
...
mod tests {
    ...
    fn geometry(x: i32) -> Geometry {
        Geometry::new(x, 20, 800, 600).unwrap()
    }
    fn created(value: u64) -> WmEvent {
        WmEvent::WindowCreated {
            id: id(value),
            geometry: geometry(value as i32),
    ...
    #[test]
    fn module_identifier_is_stable_and_backend_neutral() {
        assert_eq!(module_id().as_str(), "nexxus-wm");
    }

    #[test]
    fn duplicate_ids_are_rejected() {
        let mut wm = WindowManager::new();
        wm.apply_event(created(1)).unwrap();
        assert!(matches!(
            wm.apply_event(created(1)),
            Err(WmError::DuplicateWindow(_))
        ));
    }
    ...
    fn focus_and_mru_are_deterministic() {
        let mut wm = WindowManager::new();
        wm.apply_event(created(1)).unwrap();
        wm.apply_event(created(2)).unwrap();
        wm.set_focus(id(1)).unwrap();
        wm.set_focus(id(2)).unwrap();
    ...
    fn destroying_focused_window_recovers_previous_focus() {
        ...
        wm.set_focus(id(1)).unwrap();
        wm.set_focus(id(2)).unwrap();
        wm.apply_event(WmEvent::WindowDestroyed { id: id(2) })
            .unwrap();
        assert_eq!(wm.focused(), Some(id(1)));
        assert!(wm.window(id(1)).unwrap().active);
    }
    ...
    fn resize_obeys_constraints_without_mutating_until_backend_event() {
        ...
        .unwrap();
        assert_eq!(
            wm.request_resize(id(1), 100, 5000).unwrap(),
    ...
    fn maximize_and_restore_preserve_floating_geometry() {
        ...
        wm.apply_event(WmEvent::WindowGeometryChanged {
            id: id(1),
            geometry: Geometry::new(0, 0, 1920, 1080).unwrap(),
        })
        .unwrap();
        wm.restore(id(1)).unwrap();
        assert_eq!(wm.window(id(1)).unwrap().geometry, original);
        assert_eq!(
            wm.window(id(1)).unwrap().presentation,
    ...
    fn nested_fullscreen_restore_returns_through_previous_state() {
        ...
        wm.apply_event(created(1)).unwrap();
        wm.maximize(id(1)).unwrap();
        wm.set_fullscreen(id(1), true).unwrap();
        wm.restore(id(1)).unwrap();
        assert_eq!(
            wm.window(id(1)).unwrap().presentation,
    ...
    fn floating_state_survives_tiled_round_trip() {
        let mut wm = WindowManager::new();
        wm.apply_event(created(1)).unwrap();
        let floating = wm.window(id(1)).unwrap().geometry;
        wm.set_placement(id(1), WindowPlacement::Tiled).unwrap();
        wm.apply_event(WmEvent::WindowGeometryChanged {
            id: id(1),
            geometry: Geometry::new(0, 0, 960, 1080).unwrap(),
        })
        .unwrap();
    ...
    fn stale_events_are_ignored_after_destruction() {
        let mut wm = WindowManager::new();
        wm.apply_event(created(1)).unwrap();
        wm.apply_event(WmEvent::WindowDestroyed { id: id(1) })
            .unwrap();
        assert_eq!(
            wm.apply_event(WmEvent::WindowGeometryChanged {
                id: id(1),
    ...
    fn invalid_state_sequences_fail_without_corrupting_restore_state() {
        ...
        assert!(matches!(
            wm.restore(id(1)),
            Err(WmError::NothingToRestore(_))
        ));
        wm.maximize(id(1)).unwrap();
        assert!(matches!(
            wm.maximize(id(1)),
            Err(WmError::InvalidState { .. })
        ));
        wm.restore(id(1)).unwrap();
        assert_eq!(
            wm.window(id(1)).unwrap().presentation,
