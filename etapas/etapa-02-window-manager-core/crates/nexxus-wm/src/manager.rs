...
use crate::{
    BackendCommandSink, Geometry, GeometryError, PresentationState, Window, WindowId,
    WindowMetadata, WindowPlacement, WmCommand, WmEvent,
};
...
impl WindowManager {
    ...
    pub fn mru(&self) -> impl Iterator<Item = WindowId> + '_ {
        ...
        match event {
            WmEvent::WindowCreated {
                ...
                if self.windows.contains_key(&id) {
                    return Err(WmError::DuplicateWindow(id));
                }
    ...
    pub fn set_focus(&mut self, id: WindowId) -> Result<(), WmError> {
        let focusable = self
            .windows
            .get(&id)
            .map(|w| w.mapped && w.visible)
            .ok_or(WmError::UnknownWindow(id))?;
        if !focusable {
            return Err(WmError::InvalidState {
                window: id,
                operation: "focus",
                state: self.windows[&id].presentation,
            });
        }
        self.clear_focus();
    ...
    pub fn focus_previous(&mut self) -> Option<WindowId> {
        ...
        let next = self
            ...
            .iter()
            .copied()
            .find(|candidate| Some(*candidate) != current && self.is_focusable(*candidate));
    ...
    pub fn request_resize(
        ...
        width: u32,
        height: u32,
    ) -> Result<WmCommand, WmError> {
        let window = self.require_normal(id, "resize")?;
        let constrained = Geometry::new(window.geometry.x, window.geometry.y, width, height)?
            .constrained(window.constraints)?;
        Ok(WmCommand::RequestResize {
            window: id,
    ...
    pub fn maximize(&mut self, id: WindowId) -> Result<WmCommand, WmError> {
        ...
        window.push_restore_snapshot();
        window.presentation = PresentationState::Maximized;
    ...
    pub fn set_fullscreen(&mut self, id: WindowId, enabled: bool) -> Result<WmCommand, WmError> {
        if enabled {
            ...
            if window.presentation == PresentationState::Fullscreen {
                return Err(WmError::InvalidState {
                    window: id,
                    operation: "enter fullscreen",
                    state: window.presentation,
                });
            }
            window.push_restore_snapshot();
            window.presentation = PresentationState::Fullscreen;
            return Ok(WmCommand::RequestFullscreen {
                window: id,
                enabled: true,
            });
        }
        ...
        if state != PresentationState::Fullscreen {
            return Err(WmError::InvalidState {
                window: id,
                operation: "leave fullscreen",
                state,
            });
        }
        self.restore(id)?;
        Ok(WmCommand::RequestFullscreen {
            window: id,
            enabled: false,
        })
    }
    ...
    pub fn restore(&mut self, id: WindowId) -> Result<WmCommand, WmError> {
        let window = self.window_mut(id)?;
        let snapshot = window
            .pop_restore_snapshot()
            .ok_or(WmError::NothingToRestore(id))?;
        ...
        if snapshot.placement == WindowPlacement::Floating
            && snapshot.presentation == PresentationState::Normal
        {
    ...
    pub fn set_placement(
        ...
        if window.presentation != PresentationState::Normal {
            return Err(WmError::InvalidState {
                window: id,
                operation: "change placement",
                state: window.presentation,
            });
        }
        if placement == WindowPlacement::Floating {
            window.geometry = window.floating_geometry;
        } else if window.placement == WindowPlacement::Floating {
    ...
    pub fn request_focus(&self, id: WindowId) -> Result<WmCommand, WmError> {
        self.windows
            .contains_key(&id)
            .then_some(WmCommand::RequestFocus { window: id })
            .ok_or(WmError::UnknownWindow(id))
    }
    pub fn request_close(&self, id: WindowId) -> Result<WmCommand, WmError> {
        self.windows
            .contains_key(&id)
            .then_some(WmCommand::RequestClose { window: id })
            .ok_or(WmError::UnknownWindow(id))
    }
    pub fn dispatch(
        &self,
    ...
    pub fn update_metadata(
        ...
        self.window_mut(id)?.metadata = metadata;
        Ok(())
    }
    ...
    fn window_mut(&mut self, id: WindowId) -> Result<&mut Window, WmError> {
        self.windows.get_mut(&id).ok_or(WmError::UnknownWindow(id))
    }
    ...
    fn recover_focus(&mut self) {
        let candidate = self
            ...
            .iter()
            .copied()
            .find(|candidate| self.is_focusable(*candidate));
        if let Some(candidate) = candidate {
            let _ = self.set_focus(candidate);
        }
    }
    fn is_focusable(&self, id: WindowId) -> bool {
        self.windows.get(&id).is_some_and(|w| w.mapped && w.visible)
    }
    fn touch_mru(&mut self, id: WindowId) {
        self.remove_from_mru(id);
        self.mru.push_front(id);
    }
    fn remove_from_mru(&mut self, id: WindowId) {
