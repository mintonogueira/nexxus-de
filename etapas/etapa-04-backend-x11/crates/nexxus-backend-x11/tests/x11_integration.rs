...
fn private_dir() -> std::path::PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("nexxus-x11-stage4-{}-{nonce}", std::process::id()));
...
fn maps_moves_resizes_maximizes_fullscreens_and_restores_a_real_x11_window() {
    ...
    wait_until(|| controller.windows().unwrap().iter().any(|candidate| candidate.id == id));

    controller.focus(id).unwrap();
    controller.move_window(id, 80, 90).unwrap();
    controller.resize_window(id, 480, 300).unwrap();
    wait_until(|| controller.windows().unwrap().iter().find(|candidate| candidate.id == id).is_some_and(|candidate| {
        candidate.geometry.x == 80 && candidate.geometry.y == 90 && candidate.geometry.width == 480 && candidate.geometry.height == 300
    }));

    controller.maximize(id).unwrap();
    wait_until(|| controller.windows().unwrap().iter().find(|candidate| candidate.id == id).is_some_and(|candidate| candidate.presentation == PresentationState::Maximized));
    controller.restore(id).unwrap();
    wait_until(|| controller.windows().unwrap().iter().find(|candidate| candidate.id == id).is_some_and(|candidate| candidate.presentation == PresentationState::Normal && candidate.geometry.width == 480));

    controller.fullscreen(id, true).unwrap();
    wait_until(|| controller.windows().unwrap().iter().find(|candidate| candidate.id == id).is_some_and(|candidate| candidate.presentation == PresentationState::Fullscreen));
    controller.fullscreen(id, false).unwrap();
    wait_until(|| controller.windows().unwrap().iter().find(|candidate| candidate.id == id).is_some_and(|candidate| candidate.presentation == PresentationState::Normal));
