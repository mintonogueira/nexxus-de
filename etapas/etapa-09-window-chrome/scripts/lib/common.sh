#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 09.
...
find_privilege_command() {
    if command -v sudo >/dev/null 2>&1; then PRIVILEGE_CMD='sudo';
    elif command -v doas >/dev/null 2>&1; then PRIVILEGE_CMD='doas';
    else PRIVILEGE_CMD=''; fi
}
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 09 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-09.conf" ] || die 'manifesto da Etapa 09 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-window-chrome/Cargo.toml" ] || die 'crate nexxus-window-chrome ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Window Chrome em release'
    run_logged cargo build --workspace --release || die 'cargo build falhou'
    log_msg '[test] executando fmt, clippy, testes, rustdoc e X11/Xvfb'
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação do Window Chrome falhou'
}
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: biblioteca/runtime integrável sem payload executável independente nesta etapa'
    log_msg '[status] build/test/X11/staging concluídos; pacote nativo e instalação permanecem N/A'
}
