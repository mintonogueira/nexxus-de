...
impl ChromeMetrics {
    ...
    pub fn frame_extents(self, scale: ScaleFactor) -> FrameExtents {
        let physical =
            |logical: f32| -> u32 { (logical.max(1.0) * scale.get()).round().max(1.0) as u32 };
        FrameExtents {
...
impl TitlebarLayout {
    pub fn new(width: f32, metrics: ChromeMetrics) -> Self {
        ...
        let maximize_restore = LogicalRect::new(
            (width - button * 2.0).max(0.0),
            0.0,
            button.min(width),
            height,
        );
        let tile_fit = LogicalRect::new(
            (width - button * 3.0).max(0.0),
            0.0,
            button.min(width),
            height,
        );
