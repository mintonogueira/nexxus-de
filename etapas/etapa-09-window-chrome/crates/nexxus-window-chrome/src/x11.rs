...
use x11rb::protocol::xproto::{
    ...
    CreateGCAux, CreateWindowAux, EventMask, ImageFormat, PropMode, WindowClass,
};
...
use crate::policy::{
    DecorationDecision, DecorationHints, WindowType, decide_decoration,
};
...
impl X11ChromeAdapter {
    pub fn connect(
        ...
        let screen = conn
            ...
            .roots
            .get(screen_num)
            .ok_or_else(|| ChromeX11Error::Unavailable("selected X11 screen does not exist".into()))?;
    ...
    pub fn sync(&mut self) -> Result<(), ChromeX11Error> {
        ...
        for window in windows {
            if !window.mapped || !window.visible || window.presentation == PresentationState::Fullscreen {
    ...
    fn pointer_press(
        ...
        match target {
            HitTarget::Button(button) => {
                self.pressed = Some(PressedButton { window: owner.window, button });
        ...
        if let Some(pressed) = pressed {
            ...
            if pressed.window == owner.window && released == HitTarget::Button(pressed.button) {
                match pressed.button {
                    ChromeButton::TileFit => hooks
                        .tile_fit(owner.window)
                        .map_err(ChromeX11Error::Hook)?,
                    ChromeButton::MaximizeRestore => maximize_restore(&self.controller, owner.window)?,
    ...
    fn hit_target(&self, owner: SurfaceOwner, window: &Window, x: i32, y: i32) -> HitTarget {
        let grab = (self.metrics.resize_grab * self.scale.get()).round().max(1.0) as i32;
        match owner.kind {
            ...
            SurfaceKind::RightBorder | SurfaceKind::RightGrab => HitTarget::Resize(ResizeEdge::Right),
    ...
    fn ensure_decoration(&mut self, window: &Window) -> Result<(), ChromeX11Error> {
        ...
        let left_border = self.create_surface(WindowClass::INPUT_OUTPUT, screen.black_pixel, true)?;
        let right_border = self.create_surface(WindowClass::INPUT_OUTPUT, screen.black_pixel, true)?;
        let bottom_border = self.create_surface(WindowClass::INPUT_OUTPUT, screen.black_pixel, true)?;
        ...
        for (xid, kind) in [
            ...
            self.owners.insert(xid, SurfaceOwner { window: window.id, kind });
    ...
    fn create_surface(
        ...
        self.conn
            .create_window(
                if class == WindowClass::INPUT_ONLY { 0 } else { COPY_DEPTH_FROM_PARENT },
                xid,
                self.root,
                0,
                0,
    ...
    fn sync_decoration(&mut self, window: &Window) -> Result<(), ChromeX11Error> {
        ...
        let grab = (self.metrics.resize_grab * self.scale.get()).round().max(1.0) as u32;
        let x = window.geometry.x;
        let y = window.geometry.y;
        let width = window.geometry.width;
        let height = window.geometry.height;
        let title_width = width.saturating_add(extents.left).saturating_add(extents.right);
        ...
        self.configure(decoration.right_border, x.saturating_add(width as i32), y, extents.right, height)?;
        self.configure(
    ...
    fn configure(&self, xid: u32, x: i32, y: i32, width: u32, height: u32) -> Result<(), ChromeX11Error> {
        self.conn
            .configure_window(
                xid,
    ...
    fn redraw_with(&mut self, window: &Window) -> Result<(), ChromeX11Error> {
        ...
        let width = self.titlebar_logical_width(window.geometry);
        let frame = self.painter.render(width, &window.metadata.title, state, self.scale)?;
        self.upload_frame(decoration.titlebar, decoration.title_gc, &frame)?;
        Ok(())
    }

    fn upload_frame(&self, drawable: u32, gc: u32, frame: &nexxus_ui::Frame) -> Result<(), ChromeX11Error> {
        let width = u16::try_from(frame.size.width).map_err(|_| ChromeX11Error::DimensionOverflow)?;
        let height = u16::try_from(frame.size.height).map_err(|_| ChromeX11Error::DimensionOverflow)?;
        let format = self
            .conn
            .setup()
            .pixmap_formats
        ...
        if format.is_some_and(|format| format.bits_per_pixel == 32) {
            ...
            self.conn.clear_area(false, drawable, 0, 0, width, height).map_err(operation_error)?;
        }
        Ok(())
    }
    ...
    fn read_decoration_hints(&self, id: WindowId) -> Result<DecorationHints, ChromeX11Error> {
        ...
        let gtk = self
            .conn
            .get_property(false, xid, self.atoms.gtk_frame_extents, AtomEnum::ANY, 0, 16)
            .map_err(operation_error)?
            .reply()
            .map_err(operation_error)?;
        let motif = self
            .conn
            .get_property(false, xid, self.atoms.motif_wm_hints, self.atoms.motif_wm_hints, 0, 5)
            .map_err(operation_error)?
            .reply()
            .map_err(operation_error)?;
        ...
        let types = self
            .conn
            .get_property(false, xid, self.atoms.net_wm_window_type, AtomEnum::ATOM, 0, 16)
            .map_err(operation_error)?
            .reply()
            .map_err(operation_error)?;
    ...
    fn remove_decoration(&mut self, id: WindowId) -> Result<(), ChromeX11Error> {
        ...
        self.conn.free_gc(decoration.title_gc).map_err(operation_error)?;
