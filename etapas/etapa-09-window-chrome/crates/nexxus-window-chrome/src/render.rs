...
impl ChromePainter {
    ...
    pub fn display_list(
        ...
        list.push(DrawCommand::StrokeRect {
            rect: layout.bounds,
            color: if state.active { palette.accent } else { palette.border },
        ...
        let text_style = TextStyle::new(
            ...
            if state.active { palette.text } else { palette.text_muted },
        );
        ...
        self.push_button(&mut list, layout.tile_fit, ChromeButton::TileFit, "window-tile", state)?;
        ...
        self.push_button(&mut list, layout.close, ChromeButton::Close, "window-close", state)?;
        Ok(list)
    }
    ...
    fn push_button(
        ...
        let background = if state.pressed == Some(button) {
            ...
            if state.active { palette.surface_alt } else { palette.surface }
        };
        list.push(DrawCommand::FillRect { rect, color: background });
