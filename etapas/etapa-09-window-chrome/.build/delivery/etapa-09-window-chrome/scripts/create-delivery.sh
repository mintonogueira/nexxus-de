#!/bin/sh
# Gera a cópia portátil da Etapa 09 após as validações obrigatórias.
set -eu
...
ARCHIVE="Nexxus_Etapa09_Window_Chrome_${NEXXUS_VERSION}.tar.gz"
...
# O snapshot contém somente material versionável da própria etapa; caches,
# dist e entregas anteriores não entram recursivamente no arquivo.
(
    cd "$ROOT_DIR"
    tar -cf - \
        --exclude='./target' \
