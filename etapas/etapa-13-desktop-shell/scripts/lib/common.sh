#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 13.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 13 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-13.conf" ] || die 'manifesto da Etapa 13 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-desktop-shell/Cargo.toml" ] || die 'crate nexxus-desktop-shell ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Desktop Shell em release'
    ...
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação do Desktop Shell falhou'
}
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: runtime integrável; Session Runtime é o coordenador da sessão'
