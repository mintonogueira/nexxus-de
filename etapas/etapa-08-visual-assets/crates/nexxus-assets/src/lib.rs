...
pub enum IconContext { Actions, Places, Devices, Status, Categories, MimeTypes }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AppCategory { AudioVideo, Development, Education, Game, Graphics, Network, Office, Settings, System, Utility, Other }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ApplicationIcon<'a> { External(&'a str), Builtin(&'static IconSpec) }

pub fn icon(name: &str) -> Option<&'static IconSpec> { ICONS.iter().find(|item| item.name == name) }
pub fn wallpaper(name: &str) -> Option<&'static WallpaperSpec> { WALLPAPERS.iter().find(|item| item.name == name) }

/// Preserves a declared external icon and falls back only when it is absent.
pub fn resolve_application_icon<'a>(declared: Option<&'a str>, category: AppCategory) -> ApplicationIcon<'a> {
    if let Some(name) = declared.map(str::trim).filter(|name| !name.is_empty()) { return ApplicationIcon::External(name); }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RecolorError { InvalidUtf8, PaletteTokenMissing }
impl std::fmt::Display for RecolorError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self { Self::InvalidUtf8 => "symbolic SVG is not valid UTF-8", Self::PaletteTokenMissing => "symbolic SVG does not contain the Nexxus palette token" })
    }
}
...
pub fn recolor_symbolic_svg(svg: &[u8], rgb: [u8; 3]) -> Result<Vec<u8>, RecolorError> {
    ...
    if !source.contains(SYMBOLIC_PALETTE_TOKEN) { return Err(RecolorError::PaletteTokenMissing); }
...
mod tests {
    use super::*;
    #[test] fn preserves_external_desktop_icon() { assert_eq!(resolve_application_icon(Some("firefox"), AppCategory::Network), ApplicationIcon::External("firefox")); }
    #[test] fn category_fallback_is_deterministic() { let ApplicationIcon::Builtin(spec)=resolve_application_icon(Some(" "), AppCategory::Office) else { panic!("expected fallback") }; assert_eq!(spec.name,"applications-office"); }
    #[test] fn recolors_palette_token() { assert_eq!(recolor_symbolic_svg(br##"<path stroke="#FFFFFF"/>"##,[0x36,0xF5,0x7B]).unwrap(), br##"<path stroke="#36F57B"/>"##); }
    #[test] fn icon_names_are_unique() { for (i,left) in ICONS.iter().enumerate() { assert!(ICONS.iter().skip(i+1).all(|right| left.name!=right.name),"duplicate icon: {}",left.name); } }
}
