...
#[error(
    "shortcut '{trigger}' is already bound to '{existing}', cannot bind it to '{requested}'"
)]
...
mod tests {
    ...
    fn defaults_never_reserve_bare_f11() {
        ...
        assert!(registry.binding_for(&Trigger::parse("F11").unwrap()).is_none());
        assert!(registry.bindings().all(|(trigger, _)| !trigger.is_bare_f11()));
    }
    ...
    fn dispatches_resolved_command_target() {
        ...
        assert_eq!(sink.targets, vec![CommandTarget::Wm(WmAction::CloseFocused)]);
    }
