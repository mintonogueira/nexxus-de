...
impl X11ShortcutGrabs {
    ...
    pub fn install(
        ...
        let (conn, screen_num) =
            x11rb::connect(display).map_err(|error| X11GrabError::Unavailable(error.to_string()))?;
        let root = conn
            .setup()
            .roots
            .get(screen_num)
...
impl KeyboardMap {
    ...
    fn keycodes_for_key(&self, key: &Key) -> Result<Vec<u8>, X11GrabError> {
        let candidates = keysyms_for_key(key)
            .ok_or_else(|| X11GrabError::UnsupportedKey(key.to_string()))?;
