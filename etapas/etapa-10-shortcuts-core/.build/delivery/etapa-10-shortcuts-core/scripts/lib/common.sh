#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 10.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 10 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-10.conf" ] || die 'manifesto da Etapa 10 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-shortcuts/Cargo.toml" ] || die 'crate nexxus-shortcuts ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Shortcuts Core em release'
    ...
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação do Shortcuts Core falhou'
}
