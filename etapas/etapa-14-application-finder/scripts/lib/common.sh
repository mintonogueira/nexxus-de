#!/bin/sh
# Helpers POSIX compartilhados pelos cenários Arch e Debian da Etapa 14.
...
validate_stage_tree() {
    [ -f "$ROOT_DIR/Cargo.toml" ] || die 'Cargo.toml da Etapa 14 não encontrado'
    [ -f "$ROOT_DIR/manifests/etapa-14.conf" ] || die 'manifesto da Etapa 14 não encontrado'
    [ -f "$ROOT_DIR/crates/nexxus-app-finder/Cargo.toml" ] || die 'crate nexxus-app-finder ausente'
}
...
build_and_test_workspace() {
    log_msg '[build] compilando Application Finder em release'
    run_logged cargo build --workspace --release || die 'cargo build falhou'
    log_msg '[test] executando fmt, clippy, testes e rustdoc'
    run_logged sh "$ROOT_DIR/scripts/check.sh" || die 'validação do Application Finder falhou'
}
...
finish_non_installable_stage() {
    ...
    log_msg '[install] N/A: Finder integrável ao Session Runtime'
